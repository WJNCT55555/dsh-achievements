...
2. **分层隐私** — 基础层只读取叶子标量（工具名、成功标志、agent id、事件类型、文件路径、token 计数），**从不读取**消息正文、文件内容、错误详情、搜索结果；深度洞察层（`deepInsights`，默认关闭、opt-in）允许对消息正文正则匹配与历史会话扫描，但正文只在运行时匹配、**绝不落盘**；匿名统计层（默认关闭、opt-in）为「约 x% 用户获得」仅上报成就 id、稀有度与随机匿名安装 id，绝不包含会话/工具/文件/内容数据。默认零上传，未配置端点或未开启时零网络请求。
...
- **获得率依赖匿名样本** — 「约 x% 用户获得」来自 opt-in 匿名统计（默认关闭）：开启后上报成就 id/稀有度/匿名安装 id 到配置的端点，画廊拉取并缓存；未开启、未配置端点或端点不可达时该行不显示，且不影响任何功能。免费后端参考见 `docs/telemetry-worker.js`（Cloudflare Workers 免费层）。
...
- **非匿名遥测** — 匿名统计只在上报成就 id/稀有度/匿名安装 id 时发生（opt-in、默认关闭）；任何涉及会话内容、工具调用或路径的遥测都不引入。
