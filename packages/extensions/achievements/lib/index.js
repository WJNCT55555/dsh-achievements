...
var AchievementStateStore = class {
	...
	async load() {
		try {
			...
			const telemetryRaw = isRecord$1(record["telemetry"]) ? record["telemetry"] : {};
			return {
				schemaVersion: 1,
				counters: isRecord$1(record["counters"]) ? record["counters"] : {},
				distinct: isRecord$1(record["distinct"]) ? record["distinct"] : {},
				flags: Array.isArray(record["flags"]) ? record["flags"] : [],
				unlocked: isRecord$1(record["unlocked"]) ? record["unlocked"] : {},
				telemetry: {
...
let AchievementsService = (() => {
	...
	return class AchievementsService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _telemetryState_decorators, {
				kind: "method",
				name: "telemetryState",
				static: false,
				private: false,
				access: {
					has: (obj) => "telemetryState" in obj,
					get: (obj) => obj.telemetryState
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _setTelemetry_decorators, {
				kind: "method",
				name: "setTelemetry",
				static: false,
				private: false,
				access: {
					has: (obj) => "setTelemetry" in obj,
					get: (obj) => obj.setTelemetry
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _rates_decorators, {
				kind: "method",
				name: "rates",
				static: false,
				private: false,
				access: {
					has: (obj) => "rates" in obj,
					get: (obj) => obj.rates
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		async rates() {
			...
			const now = Date.now();
			if (this.ratesCache !== null && now - this.ratesCache.at < RATES_CACHE_MS) return this.ratesCache.data;
			try {
				...
				const pct = {};
				if (users > 0) {
					for (const [id, value] of Object.entries(counts)) if (typeof value === "number") pct[id] = Math.round(value / users * 100);
				}
				const data = {
					users,
					pct
		...
		*/
		reportUnlock(a) {
			...
			fetch(`${this.telemetryEndpoint}/unlock`, {
				method: "POST",
