...
import { z } from 'zod';
import { AchievementStateStore } from "./state.js";
...
const RUNNABLE_EXTS = new Set(['.py', '.js', '.mjs', '.cjs', '.ts', '.tsx', '.sh', '.bash', '.ps1', '.go', '.rs', '.rb', '.php', '.java', '.kt']);
const SHELL_TOOLS = new Set(['bash', 'shell', 'pwsh', 'powershell']);
/** Loader module-name fragments that identify an installed dsh-deep-whale skin. */
const DEEP_WHALE_NAMES = ['dsh-client-ui-skin-maid-atelier', 'dsh-deep-whale', 'maid-atelier'];
...
let AchievementsService = (() => {
    ...
    return class AchievementsService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _heatmap_decorators, { kind: "method", name: "heatmap", static: false, private: false, access: { has: obj => "heatmap" in obj, get: obj => obj.heatmap }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _telemetryState_decorators, { kind: "method", name: "telemetryState", static: false, private: false, access: { has: obj => "telemetryState" in obj, get: obj => obj.telemetryState }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _setTelemetry_decorators, { kind: "method", name: "setTelemetry", static: false, private: false, access: { has: obj => "setTelemetry" in obj, get: obj => obj.setTelemetry }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _rates_decorators, { kind: "method", name: "rates", static: false, private: false, access: { has: obj => "rates" in obj, get: obj => obj.rates }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        static inject = ['tools'];
        counters = (__runInitializers(this, _instanceExtraInitializers), new Map());
        distinct = new Map();
        flags = new Set();
        turnState = new Map();
        unlocked = new Map();
                ...
                if (users > 0) {
                    for (const [id, value] of Object.entries(counts)) {
                        if (typeof value === 'number')
                            pct[id] = Math.round((value / users) * 100);
                    }
                }
                const data = { users, pct };
        ...
        applyPersisted(state) {
            ...
            if (state.telemetry.anonymousId !== '')
                this.anonymousId = state.telemetry.anonymousId;
        }
        ...
        snapshot() {
            ...
            const unlocked = {};
            for (const [id, at] of this.unlocked)
                unlocked[id] = at;
            return {
                schemaVersion: 1,
                counters,
        ...
        dock() {
            ...
            for (const a of ACHIEVEMENTS) {
                if (this.unlocked.has(a.id) || a.rule.kind === 'flag')
                    continue;
                const p = this.progressOf(a.rule);
                const gap = p.target - p.current;
                if (next === null || gap < next.gap)
                    next = { name: a.name, icon: a.icon, current: p.current, target: p.target, gap };
            }
            return {
                unlocked: this.unlocked.size,
                total: ACHIEVEMENTS.length,
        ...
        ruleMet(rule) {
            if (rule.kind === 'counter')
                return (this.counters.get(rule.key) ?? 0) >= rule.target;
            if (rule.kind === 'distinct')
                return (this.distinct.get(rule.key)?.size ?? 0) >= rule.target;
            if (rule.kind === 'lang-count')
                return this.maxLangCount() >= rule.target;
            return this.flags.has(rule.flag);
        }
        maxLangCount() {
            let max = 0;
            for (const [key, set] of this.distinct) {
                if (key.startsWith('lang:') && set.size > max)
                    max = set.size;
            }
            return max;
        }
        ...
        checkAll() {
            let changed = false;
            for (const a of ACHIEVEMENTS) {
                if (this.unlocked.has(a.id) || !this.ruleMet(a.rule))
                    continue;
                const at = Date.now();
        ...
        seedSessions(ctx) {
            ...
            for (const agent of agents.list()) {
                if (typeof agent.id === 'string')
                    this.addDistinct('sessions', agent.id);
            }
            this.checkAll();
        }
        ...
        detectDeepWhale(ctx) {
            ...
            for (const entry of loader.entries()) {
                ...
                if (typeof name !== 'string')
                    continue;
                if (isDeepWhaleName(name))
                    deepWhaleSeen = true;
                // Non-official rows (not @deepseek-ai/dsh-*) count as third-party plugins.
                if (!name.startsWith('@deepseek-ai/dsh-') && !name.startsWith('cordis:') && !name.startsWith('.'))
                    extraPlugins += 1;
            }
            if (deepWhaleSeen)
                this.mark('deepWhale');
            if (extraPlugins >= 5)
                this.counters.set('extraPlugins', extraPlugins);
            this.checkAll();
        }
        attachListeners(ctx) {
            ctx.on('tools/result', (exec, result) => {
                ...
                if (!result.isError)
                    ...
                    this.counters.set('streak', 0);
                const turn = this.turnFor(exec.agent);
                turn.toolCalls += 1;
                turn.distinctTools.add(name);
                if (turn.toolCalls >= 10)
                    this.mark('marathon');
                if (turn.distinctTools.size >= 8)
                    this.mark('toolPalette');
                ...
                if ((name === 'write' || name === 'edit') && exec.arguments !== undefined) {
                    ...
                    if (typeof path === 'string') {
                        ...
                        if (RUNNABLE_EXTS.has(ext))
                            turn.wroteRunnable = true;
                    }
                }
                if (SHELL_TOOLS.has(name))
                    turn.ranShell = true;
                if (turn.wroteRunnable && turn.ranShell)
                    this.mark('thatWorks');
                this.checkAll();
            });
            ...
            ctx.on('session/event', (_session, event) => {
                if (event.type === 'assistant/message') {
                    ...
                    if (usage !== undefined && typeof usage === 'object') {
                        ...
                        if (!this.seenUsage.has(key)) {
                            ...
                            if (total > 0)
                                this.bump('tokens', total);
                            if (typeof record.outputTokens === 'number')
                                this.bump('outTokens', record.outputTokens);
                            if (typeof record.cacheReadTokens === 'number')
                                this.bump('cacheRead', record.cacheReadTokens);
                            if (typeof record.inputTokens === 'number')
                                this.bump('uncachedInput', record.inputTokens);
                            if (typeof record.reasoningTokens === 'number')
                                this.bump('reasoningTokens', record.reasoningTokens);
                            this.checkAll();
                ...
                if (event.type === 'request/header') {
                    ...
                    if (typeof provider === 'string' && provider.length > 0)
                        this.addDistinct('providers', provider);
                    if (typeof model === 'string' && model.length > 0)
                        this.addDistinct('models', model);
                    return;
                }
                ...
                if (event.type === 'approval/decided') {
                    if (event.data.outcome === 'rejected')
        ...
        attachDeepListeners(ctx) {
            ctx.on('session/event', (_session, event) => {
                ...
                if (DEEP_BODY_PATTERNS['deep-sorry']?.test(text))
                    this.bump('deepSorry');
                if (DEEP_BODY_PATTERNS['deep-code-heavy']?.test(text))
                    this.bump('deepCodeHeavy');
                this.checkAll();
            });
        }
                    ...
                    for (const event of session.events) {
                        if (event.type === 'assistant/message') {
                            ...
                            if (typeof header?.config?.provider === 'string')
                                this.addDistinct('providers', header.config.provider);
                            if (typeof header?.config?.model === 'string')
                                this.addDistinct('models', header.config.model);
                        }
